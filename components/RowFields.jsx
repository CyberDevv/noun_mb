...
export const RowName = ({ label, naration }) => (
  <div className="space-x-3 start">
    ...
    <div>
      ...
      <p
        className={`${
          naration === "debit" ? "text-F7" : "text-00G"
        } capitalize text-[10px] leading-[14px] tracking-[0.3px] font-inter mt-0.5`}
      >
        {naration}
      </p>
    </div>
...
export const RowStatusType2 = ({ label }) => (
  <div>
    <p
      className={`${
        label === "ENABLED"
          ? "text-00G border-[#0FB48559]"
          : "text-[#D32F2F] border-[#D32F2F59]"
      } font-medium leading-[20px] border capitalize rounded-full w-fit px-2 py-1 start space-x-1 mx-auto`}
    >
      <span>{label?.toLowerCase()}</span>
    </p>
  </div>
...
export const RowVerification = ({ label }) => (
  <p
    className={`${
      label === "Approved" ? "text-00G bg-E6" : "bg-[#FEEDED] text-[#D32F2F]"
    }  leading-[20px] text-sm font-medium font-inter py-px px-2 rounded-full w-fit`}
  >
    {label}
  </p>
);
